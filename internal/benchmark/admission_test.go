...
func TestAdmitRunUsesExplicitCeilingsWithoutDerivingCapacity(t *testing.T) {
	...
	decision, err := AdmitRun(AdmissionRequest{Concurrency: 512, Requests: 3, MaxConcurrency: 512, MaxRequests: 3}, diagnostics)
	if err != nil {
		t.Fatalf("AdmitRun: %v", err)
	}
	if decision.WorkerCount != 3 || decision.Diagnostics != diagnostics {
