...
func TestRunnerReturnsContentBearingObservation(t *testing.T) {
	runner := NewRunner(executorFunc(func(_ context.Context, request Request, observation *RequestObservation) error {
		observation.StreamEvents = append(observation.StreamEvents, StreamEvent{
			Sequence: 1, ReceivedAt: time.Now(), ReceivedAfterNS: 1, HasContent: true, ContentBytes: 5,
		})
		return nil
	}))

	result := runner.RunRequest(context.Background(), Request{RunID: "run-001", RequestID: "req-000001"})
	if result.Err != nil {
		t.Fatalf("RunRequest returned error: %v", result.Err)
	}
	if result.Observation.RunID != "run-001" || result.Observation.RequestID != "req-000001" || result.Observation.Error != "" {
